...
public class ObserverConnectionEvent {
    ...
    public static void broadcastObserverList(MinecraftServer server) {
        ...
        List<ObserverListPacket.ObserverEntry> entries = new ArrayList<>();

        for (ObserverSessionManager.Session session : ObserverSessionManager.getSessions()) {
            UUID uuid = ObserverTabListManager.uuidFromName(session.observer.getName());
            int latency = ObserverLatency.calculate(session);
